...
exports.viewSnippetsLanguage = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Snippets found", categorizedSnipp: categorizedSnipp });
  } catch (error) {
    res.status(400).json({ error: error });
  }
};
...
exports.downvoteSnippet = async (req, res) => {
  try {
    ...
    await Snippet.findOneAndUpdate({_id: req.body._id},{ downvote: newDownvoted });
...
exports.favouriteSnippet = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "favourite mark successfully", fav: favourite });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
